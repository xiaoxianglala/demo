...
public class StudentResource {
    ...
    @GetMapping("/students/stuage")
    @Timed
    public ResponseEntity<List<Student>> getStus(@RequestParam(value = "age1") Integer age1,@RequestParam(value = "age2") Integer age2){
        List<Student> students=studentService.getStus(age1,age2);
        return ResponseEntity.ok().body(students);
    }

    @GetMapping("/students/stuage/{age1}_{age2}")
    @Timed
    public ResponseEntity<List<Student>> getStudents(@PathVariable Integer age1,@PathVariable Integer age2){
        List<Student> students=studentService.getStus(age1,age2);
        return ResponseEntity.ok().body(students);
    }
    @GetMapping("studengtd/stunames")
    @Timed
    public ResponseEntity<List<Student>> getStusInfo(@RequestParam(value="stuName") String stuName){
        List<Student> list=studentService.getStusInfo(stuName);
        return ResponseEntity.ok().body(list);
    }
    @GetMapping("/student/stuages")
    @Timed
    public ResponseEntity<List<Student>> getStusInfoBySex(@RequestParam(value="stuSex") String stuSex){
        List<Student> list=studentService.getStusInfoBySex(stuSex);
        return ResponseEntity.ok().body(list);
    }
}
