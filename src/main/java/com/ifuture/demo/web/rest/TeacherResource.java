...
public class TeacherResource {
    ...
    @GetMapping("/teacher/teasname")
    @Timed
    public ResponseEntity<List<Teacher>> getTeasInfo(@RequestParam(value="teaName") String teaName){
        List<Teacher> list=teacherService.getTeasInfoByName(teaName);
        return ResponseEntity.ok().body(list);
    }
    @DeleteMapping("/teachers/{name}")
    @Timed
    public ResponseEntity<Void> deleteTeacherByName(@PathVariable String name){
        log.debug("REST request to delete Teacher : {}",name);
        teacherService.deleteTeacherByName(name);
        return ResponseEntity.ok().headers(HeaderUtil.createEntityCreationAlert(ENTITY_NAME,name.toString())).build();
